...
<Step title="Test with the native client first">
If the database is slow to respond, verify the same host, port, and credentials with the native client (`psql`, `mysql`, `mongosh`, and so on). If the native client also hangs or times out, the issue is network reachability, database health, or an upstream timeout rather than a configurable WhoDB web setting.
</Step>
</Steps>
...
The web app does not create SSH tunnels from the login form in this codebase. Use one of these approaches:
...
After creating the tunnel, connect WhoDB to the forwarded endpoint:
- Host: `localhost` (or the host where your forward terminates)
- Port: the forwarded local port, such as `5432`
- Database Host/Port inside the `ssh -L` command should target the real database

From WhoDB's perspective, it only needs the final reachable host and port.
</Step>
...
- Export in batches
- Use native database tools for full dumps or migration archives
...
- Run large generation jobs in smaller batches
</Step>
...
- Simplify query
- Check database-side or proxy timeouts if the same query dies outside WhoDB
</Step>
...
- Hover over the cell and use the add-cell control
