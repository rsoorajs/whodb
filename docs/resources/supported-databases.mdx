...
| `MongoDB` | MongoDB | Network | No | Yes | Uses document-oriented browsing instead of SQL scratchpad |
| `FerretDB` | MongoDB-compatible | Network | No | Yes | Uses MongoDB-family behavior |
| `DocumentDB` | MongoDB-compatible | Network | No | Yes | AWS-managed, shown through provider discovery |
| `Redis` | Redis | Network | No | No | Key/value browsing workflow |
| `Valkey` | Redis-compatible | Network | No | No | Uses Redis-family behavior |
| `Dragonfly` | Redis-compatible | Network | No | No | Uses Redis-family behavior |
| `ElastiCache` | Redis-compatible | Network | No | No | AWS-managed, shown through provider discovery |
...
- Document, cache, and search systems use narrower capability sets
- Database switching and schema support vary by family

### Build-Specific Availability

DuckDB is registered conditionally in this repository and is not available on every build target.
