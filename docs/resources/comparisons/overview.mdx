...
<Note>
This comparison reflects the current CE build in this repository.
</Note>

| Feature | WhoDB | phpMyAdmin | pgAdmin | DBeaver | TablePlus |
|---------|-------|------------|---------|---------|-----------|
| **Price** | Free (Open Source) | Free | Free | Free / $199 | $89-$129 |
| **Resource Usage** | Lightweight server | Moderate web stack | Heavier admin app | Heavy desktop IDE | Native desktop app |
| **Startup Model** | Run server, open browser | Web/PHP app | Desktop or server app | Java desktop app | Native desktop app |
| **Web-Based** | Yes | Yes | Desktop/Web | Desktop | Desktop |
| **Multi-Database** | Yes (broad CE catalog) | MySQL only | PostgreSQL only | Yes (50+) | Yes (15+) |
...
| **Search / Filtering** | Loaded-row search + WHERE filters | Limited | Limited | Yes | Yes |
...
**Limitations:**
- Heavier desktop runtime
- More startup and workspace overhead than lightweight web tools
- Complex interface
...
**WhoDB**: Interactive graph view with relationship visualization, pan/zoom, node cards, and Data buttons
...
**WhoDB**: Multi-cell scratchpad with syntax highlighting, query history, and one-click reuse
...
**WhoDB**: Add row, edit row, delete row, and selected-row actions through table controls and sheets
...
**WhoDB**: CSV and Excel for tables, plus JSON Lines for MongoDB-style records when supported
...
**WhoDB**: Built-in mock data generation when the connected source and object support it
...
<Accordion title="Large Organization">
**Recommended: WhoDB CE or DBeaver paid editions, depending on the workflow**

Large organizations need:
- **WhoDB CE**: Best for standardized, web-based access when the current CE source catalog fits
- **DBeaver paid editions**: Best for power users needing maximum database coverage

Consider WhoDB CE if you want:
- Central web-based access
- Easier IT management
- Lower training costs
- Security handled through your existing network, auth, and database controls

Consider DBeaver paid editions if you need:
...
<AccordionGroup>
  ...
  <Accordion title="Freemium / Paid Desktop">
    **DBeaver**
    - Community Edition: Free
    - Paid editions add broader drivers, team features, and vendor support
...
2. Copy important SQL into Scratchpad pages, CLI bookmarks, or your repository
3. Recreate commonly-used workflows in WhoDB
...
Paid DBeaver editions are a better fit for large teams with diverse database needs and dedicated training.
</Accordion>
...
- **DBeaver paid editions**: Broader driver support, team features, or vendor support
...
For stricter security needs, run WhoDB behind your existing auth layer and use database-native auditing plus infrastructure logs.
</Accordion>
</AccordionGroup>
