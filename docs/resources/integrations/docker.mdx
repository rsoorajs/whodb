...
# AI Integration
WHODB_ANTHROPIC_API_KEY=sk-ant-api03-xxx
WHODB_OPENAI_API_KEY=sk-xxx

# Ollama Configuration
WHODB_OLLAMA_HOST=ollama-server
WHODB_OLLAMA_PORT=11434

# Optional: serve WhoDB under https://example.com/whodb/
...
### Nginx Reverse Proxy on a Subpath

To serve WhoDB from a subpath such as `https://example.com/whodb/`, set `WHODB_BASE_PATH` on the WhoDB container and proxy that same prefix in Nginx.

This only applies when WhoDB serves its bundled frontend, such as the published Docker image. It does not apply to split dev mode.
...
- Browse to `https://example.com/whodb/`
- Set `WHODB_BASE_PATH=/whodb` in the WhoDB container
- Keep health checks pointed at `http://whodb:8080/health`
...
services:
  whodb:
    ...
    environment:
      ...
      - WHODB_LOG_FILE=default
      - WHODB_ACCESS_LOG_FILE=default
      # Optional: serve the bundled app from /whodb/
      - WHODB_BASE_PATH=/whodb
    env_file:
      - .env.production
    volumes:
