...
**4. Check database or proxy timeout settings:**
If the same query dies outside WhoDB, review the timeout that actually applies in your stack such as PostgreSQL `statement_timeout`, a reverse proxy read timeout, or a CLI profile timeout.
...
**3. Break the job into smaller batches:**
- Lower the page size in [Settings](/features/settings)
- Add filters or date ranges
- Export specific columns instead of entire rows
...
**1. Reduce concurrency and check database-side limits:**
The current CE web app uses an internal connection pool and does not expose `DB_MAX_*` web env vars. If you see connection-limit errors:
- Check the database server's own `max_connections` or equivalent
- Reduce the number of concurrent heavy operations
- Add a database-side pool or proxy if many services share the same database
